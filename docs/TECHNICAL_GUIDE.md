...
![Главное окно DnDEditor](../site/img/screenshot_main_dnd.png)
*Рис. 1. Основной интерфейс редактора: панель быстрых тегов сверху, текстовое поле с подсветкой, строка состояния снизу.*
...
![Подсветка тегов](../site/img/screenshot_tag_highlight.png)
*Рис. 2. Автоматическая цветовая подсветка тегов: [ЛОКАЦИЯ] (синий), [СОБЫТИЕ] (оранжевый), [ЛУТ] (зелёный), [СЕКРЕТ] (фиолетовый).*
...
![Генератор бросков](../site/img/screenshot_dice_throwing.png)
*Рис. 3. Диалоговое окно выбора типа кубика (d4–d100) и отображение результата броска.*
...
Для ускорения работы мастера реализована система шаблонов, доступная через меню «Шаблоны»:

![Меню шаблонов](../site/img/screenshot_scene_templates.png)
*Рис. 4. Выпадающее меню с готовыми структурами для Локаций, NPC, Боевых сцен, Лута и Диалогов.*

При выборе шаблона в текст автоматически вставляется структурированный блок с предзаполненными тегами и Markdown-разметкой.

### Экспорт в HTML

Редактор поддерживает экспорт сценария в формат HTML с сохранением цветовой схемы тегов:

![Экспорт в HTML](../site/img/screenshot_html_export.png)
*Рис. 5. Результат экспорта сценария в браузер: теги сохраняют свои цвета, текст форматируется согласно структуре.*
...
5. [Python Encoding HOWTO](https://docs.python.org/3/howto/unicode.html)

---

## 10. Лицензия и контактная информация
